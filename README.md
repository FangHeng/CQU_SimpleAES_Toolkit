...
在加密过程中对密文进行篡改，发现其效果并不明显，仅在更改的块中有影响。得出结论：在没有其他安全措施的情况下，加密过程中的篡改是不安全的，加密算法需要保证连续性。

在加密完成后对密文分组进行篡改，发现密文在形成后的篡改会引起后一个块的更改。得出结论：在没有其他安全措施的情况下，这种篡改依然是不安全的，攻击的精度很高。
